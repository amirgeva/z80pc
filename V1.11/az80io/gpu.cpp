    ...
    for(int iy=0;iy<15;++iy)
    {
        for(int ix=0;ix<20;++ix, ++color)
        {
            for(int i=0;i<32;++i)
            {
                int y=iy*32+i;
                int x=ix*32;
                uint8_t* row = vga.getLineBuffer(y)+x;
                for(int j=0;j<32;++j)
                {
                    *row++ = color;
                }
...
static_assert(sizeof(Character)==CHAR_HEIGHT, "Invalid character struct size");
...
void gpu_text(uint16_t& x, uint16_t& y, const char* text, uint8_t n, uint8_t fg_color, uint8_t bg_color)
{
    ...
    for (; (c = *buffer) != 0; ++buffer)
    {
        ...
        gpu_draw_char(x,y,c,fg_color,bg_color);
        x+=CHAR_WIDTH;
    }
}
