...
#define COMMAND_ACQUIRE      6
#define COMMAND_RELEASE      7
...
void parse_msg()
{
    ...
    if (msg.payload.cmd == COMMAND_AUTO_CLOCK)
    {
      AutoClock();
		...
		if (msg.payload.cmd == COMMAND_READ)
		{
      sprintf(dbgmsg,"Reading from %x len %x",addr,len);
      debug_print(dbgmsg);
    ...
    if (msg.payload.cmd == COMMAND_ACQUIRE)
    {
      AcquireBUS();
    }
    if (msg.payload.cmd == COMMAND_RELEASE)
    {
      ReleaseBUS();
    }
	}
