...
class Mega:
    def __init__(self):
        ...
        # self.send_message(create_acquire_message())

    def shutdown(self):
        self.send_message(create_release_message())
    ...
    def write_memory(self, address, data):
        ...
        i = 0
        while i < n:
            print(f"Sending address {address}")
            cur = min(n - i, 52)
            block = data[i:(i + cur)]
    ...
    def read_memory(self, address: int, length: int):
        ...
        result = []
        while i < length:
            print(f"Sending address {address + i}")
            cur = min(length - i, 52)
...
def unit_test():
    ...
    while True:
        ...
        if parts[0] == 'w':
            address = int(parts[1])
            data = [int(i) for i in parts[2:]]
            n = len(data)
            mega.write_memory(address, data)
        if parts[0] == 'r':
            address = int(parts[1])
            n = int(parts[2])
            data = mega.read_memory(address, n)
            print_hex(data)
        if parts[0] == 'u':
            address = int(parts[1])
            try:
                data = open(parts[2], 'rb').read()
                print(f'Uploading memory block of size {len(data)}')
                mega.write_memory(address, data)
            except OSError:
