...
''')
        for name in asm_files:
            ...
            o.write(f'{base}.rel: {name}\n\tsdasz80 -l -o {base}.rel {name}\n\n')
        for name in c_files:
            base, _ = os.path.splitext(os.path.basename(name))
            o.write(f'{base}.rel: {name}\n\tsdcc -c $(CFLAGS) -mz80 {name}\n\n')
        o.write('''clean:
\trm -f *.asm
\trm -f *.map
\trm -f *.rel
