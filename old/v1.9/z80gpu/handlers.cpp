...
void handle_single_step(const uint8_t* unused_data, uint8_t unused_length)
{
	...
	if (is_bus_acquired())
	{
		//g_DebugProtocol.print_text("Rel Bus");
		release_bus();
		//g_DebugProtocol.print_text("Bus Reld");
	}
	//else g_DebugProtocol.print_text("--- Single ---");
	uint8_t addrh,addrl;
	uint8_t dbyte, flags=0xFF;
	//uint8_t stage=0;
	uint8_t* buffer = g_DebugProtocol.allocate_read_response(16);
	//if (g_EnhanceSingle) 
	read_bus(addrl, addrh, dbyte, flags);
	if (!g_EnhanceSingle)
	{
		...
		{
			clock_tick();
			//g_DebugProtocol.print_text("Read bus");
			read_bus(addrl, addrh, dbyte, flags);
		}
		buffer[0]=addrl;
		buffer[1]=addrh;
