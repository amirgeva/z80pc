...
uint16_t GPUProtocol::get_header_len()
{
...
uint16_t GPUProtocol::check_variable_len()
{
    switch (Header.cmd.opcode)
    {
...
bool GPUProtocol::process_command()
{
    switch (Header.cmd.opcode)
    {
        case CMD_NOP:
        ...
        case CMD_FILL_RECT:
        {
            gpu_fill_rect(m_CursorX, m_CursorY,
                          Header.fill_rect.w,
                          Header.fill_rect.h,
                          m_BGColor);
            return true;
        ...
        case CMD_HORZ_LINE:
        {
            gpu_line(m_CursorX, m_CursorY, m_CursorX + Header.horz_line.w - 1, m_CursorY, m_FGColor);
            m_CursorX += Header.horz_line.w;
            return true;
        }
        case CMD_VERT_LINE:
        {
            gpu_line(m_CursorX, m_CursorY, m_CursorX, m_CursorY + Header.vert_line.h - 1, m_FGColor);
        ...
        case CMD_HORZ_PIXELS:
        {
            ...
            gpu_pixels(m_CursorX, m_CursorY, data, Header.horz_pixels.n);
            return true;
        }
        case CMD_TEXT:
        {
            ...
            Serial.println(text);
			*/
            gpu_text(m_CursorX, m_CursorY, text, Header.text.n, m_FGColor, m_BGColor);
        ...
        case CMD_SET_SPRITE:
        {
            ...
            gpu_set_sprite(Header.set_sprite.id, data);
            return true;
        }
        case CMD_DRAW_SPRITE:
        {
            gpu_draw_sprite(Header.draw_sprite.id, m_CursorX, m_CursorY);
            return true;
        }
    }
    return false;
...
void GPUProtocol::add_byte(uint8_t b)
{
    if (m_Pos == 0)
    {
        Header.cmd.opcode = b;
    ...
    if (m_BytesLeft == 0) 
	{
		m_BytesLeft = check_variable_len();
	}
    if (m_BytesLeft == 0)
    {
        process_command();
